...
const getPosts = async (req, res, next) => {
    try {
        let posts = await db.any("SELECT posts.*, ARRAY_AGG(comments.*)AS comments FROM posts JOIN comments ON comments.post_id = posts.id GROUP BY posts.id ORDER BY users_id");
        res.status(200).json({
            posts,
            status: "Success!",
            message: "ALL posts!"
